...
const TaskModal = ({isOpen, onClose , taskToEdit, onSave, onLogout}) => {
    ...
    const handleSubmit = useCallback(async (e)=>{
        ...
        try{
            ...
            console.error(err)
            setError(err.message || 'An unexpexted error occured')
        }finally{
            setLoading(false)
        }
    ...
    <div className='fixed inset-0 backdrop-blur-sm bg-black/20 z-50 flex items-center justify-center'>
        <div className='bg-white border border-purple-100 rounded-xl max-w-md w-full shadow-lg relative p-6 animate-fadeIn'>
            <div className='flex justify-between items-center mb-6'>
                ...
                <button onClick={onClose} className='p-2 hover:bg-purple-100 rounded-lg transition-colors text-gray-500 hover:text-pruple-700'>
                        <X className='w-5 h-5'/>
                </button>
            </div>
                ...
                {error && <div className='text-sm text-red-600 bg-red-50 p-3 rounded-lg border border-red-100'>
                   ...
                   <div >
                        ...
                        <div className='flex items-center border border-purple-100 rounded-lg px-3 py-2.5 focus-within:ring-2 focus-within:ring-purple-500 focus-within:border-purple-500 transition-all duration-200'>
                            <input type='text' name='title' required balue={taskData.title} onChange={handleChange} className='w-full focus:outline-none text-sm ' placeholder='Enter task title' />     
                        </div>
                   </div>
                   ...
                   <div>
                     <label className='flex items-center gap-1 text-sm font-medium text-gray-700 mb-1'>
                        ...
                        placeholder='Add detailes about your task' />
                   </div>
